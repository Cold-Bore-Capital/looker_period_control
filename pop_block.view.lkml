...
dimension: current_date {
  ...
  sql: {% if exclude_days._parameter_value == "999" %}
       ...
       {% elif exclude_days._parameter_value == "999" or exclude_days._parameter_value == "0" %}
           ${getdate_func}
       {% else %}
          -- Any date exclusions calc here
          dateadd('day', -${exclude_days_dim}, ${getdate_func})
       {% endif %};;
  ...
  dimension: period_1_start {
    ...
    sql:
      {% case current_period_selection._parameter_value%}
        {% when "custom_days" %}
          -- Selection for a custom number of days. Usually this would be used in a trailing scenario.
          dateadd('day', -(${size_of_range_dim} - 1) , ${current_date})

        {% when "mtd" %}
          -- Month to Date. Find the first day of the month.
          date(date_trunc('month', ${current_date}))

        {% when "qtd" %}
          -- Quarter to Date. Find the first day of the quarter.
          date(date_trunc('quarter', ${current_date}))

        {% when "ytd" %}
          -- Year to Date. Find the first day of the year.
          date(date_trunc('year', ${current_date}))

        {% when "pm" %}
          -- Prior Month. Find the first day of last month.
          date(date_trunc('month', date_add('months', -1, ${current_date})))

        {% when "pq"%}
          -- Prior Quarter. Find the first day of last quarter.
         date(date_trunc('quarter', date_add('quarters', -1, ${current_date})))

        {% when "py" %}
          -- Prior Year. Find the first day of last year
          date(date_trunc('year', date_add('years', -1, ${current_date})))
  ...
  dimension: period_1_end {
    ...
    sql:
      ...
      {% case current_period_selection._parameter_value%}
        {% when "custom_days" or "mtd" or "qtd" or "ytd" %}
          -- Just get the current period end date
            ${current_date}
        ...
        {% when "pq"%}
          -- Prior Quarter. Find the first day of last quarter.
         date(date_add('days', -1, date_trunc('quarter', ${current_date})))

        {% when "py" %}
          -- Prior Year. Find the first day of last year
          date(date_add('days', -1, date_trunc('year', ${current_date})))
  ...
  dimension: period_2_start {
    ...
    {% case compare_period_selection %}
      {% when "trailing" %}
        -- Multiply the size the range * 2, then subtract from current date. Extra -1 to move back one day.
        dateadd('day', -((${size_of_range_dim} * 2) - 1), ${current_date})

      {% when "prior_month" or "prior_quarter"%}
        date(date_add('days', -(date_diff('days', ${period_1_start}, ${period_1_end})+1), ${period_1_start}))

      {% when "prior_year" %}
        date(date_trunc('year', date_add('years', -1, ${period_1_start})))
  ...
  #   type: date_raw
  #   sql:
  #     {% if compare_to._parameter_value == "tp_ly" %}
  ...
  #   type: date_raw
  #   sql:    {% if compare_to._parameter_value == "tp_ly" %}
  #             {% if exclude_days._parameter_value == "999" %}
  ...
  #   }
  #   allowed_value: {
  #     label: "This Period Last Month (MTD)"
  #     value: "tp_mtd"
  #   }
  #   allowed_value: {
  #     label: "Prior Month Trailing (Completed Month)"
  #     value: "lm_tma"
  #   }
  #   allowed_value: {
  #     label: "Prior Month Last Year (Completed Month)"
  #     value: "lm_ly"
  #   }
  #   allowed_value: {
  #     label: "This Period Last Quarter (QTD)"
  #     value: "qoq_tp"
  #   }
  #   allowed_value: {
  #     label: "Prior Quarter (Completed Quarter)"
  #     value: "qoq"
  #   }
  #   allowed_value: {
  #     label: "Prior Quarter Last Year (Completed Quarter)"
  #     value: "qoq_ly"
  #   }
  #   allowed_value: {
  #     label: "This Period Last Year (YTD)"
  #     value: "yoy_tp"
  #   }
  #   allowed_value: {
  #     label: "Year over Year (Completed Year)"
  #     value: "tp_ly"
  #   }
  # }

  parameter: current_period_selection {
    label: "1.1 Current Period Selection"
    view_label: "Timeline Comparison Fields"
    type: unquoted
    allowed_value: {
      label: "Custom Days Size (fill in days)"
      value: "custom_days"
    }
    allowed_value: {
      label: "Month to Date"
    ...
    default_value: "custom_days"
  }

  dimension: current_period_selection_dim {
    hidden: yes
    type: string
    sql: ${current_period_selection} ;;
  }

  parameter: compare_period_selection {
    label: "1.2 Compare to Period Selection"
    view_label: "Timeline Comparison Fields"
    type: unquoted
    allowed_value: {
      label: "Trailing"
  ...
  dimension: compare_period_selection_dim {
    hidden: yes
    type: string
    sql: ${compare_period_selection} ;;
  }
